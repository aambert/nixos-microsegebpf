...
| Multi-level wildcards (`*.*.example.com`) | ❌ rejected at parse time | DNS doesn't standardise two-level wildcards in the SNI sense. The agent rejects any pattern with more than one `*` rather than silently accepting an ambiguous match. |
| Subject Alternative Names (SAN) | ❌ (different layer) | SAN lives in the **server's** certificate (sent in ServerHello/Certificate), not in the client's ClientHello. Matching it would require parsing ASN.1/DER from the server response — useful for *audit* of "this server presented an unexpected cert", not for *prevention* (the handshake is already in progress when the cert arrives). For prevention, SNI is what you want. |
...
Six worked examples covering the most common workstation-hardening
shapes. Each one is a complete `services.microsegebpf.policies`
fragment you can drop into your deployment flake.
...
- **Map flush on Apply** is a full sweep, not a delta. Acceptable
  at workstation scale (a few thousand entries) but not for a
  router-class device.
...
- Delta-based map updates instead of flush-and-fill
