...
| Wildcards multi-niveaux (`*.*.example.com`) | ❌ rejeté au parse | DNS ne standardise pas les wildcards à deux niveaux côté SNI. L'agent rejette tout pattern avec plus d'un `*` plutôt que d'accepter silencieusement un match ambigu. |
| Subject Alternative Names (SAN) | ❌ (autre couche) | SAN vit dans le certificat du **serveur** (envoyé dans ServerHello/Certificate), pas dans le ClientHello du client. Le matcher demanderait de parser l'ASN.1/DER de la réponse serveur — utile pour de l'*audit* (« ce serveur a présenté un cert inattendu »), pas pour de la *prévention* (le handshake est déjà en cours quand le cert arrive). Pour la prévention, c'est SNI qu'il faut. |
